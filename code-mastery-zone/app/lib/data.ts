...
export const courseTranslations: Record<string, {
  ...
  "أساسيات البرمجة للمبتدئين": {
    ...
    en: {
      ...
      description: "A comprehensive course to learn the basics of programming from scratch. You will learn the fundamental concepts you need to get started in any programming language with practical applications."
    }
  },
  // كورس بايثون
  "مغامرات بايثون": {
    ar: {
      title: "مغامرات بايثون",
      description: "أتقن اللغة الأكثر شعبية من خلال بناء مغامرات نصية وأدوات."
    },
    en: {
  ...
  "سحر الويب": {
    ar: {
      title: "سحر الويب",
      description: "أنشئ مواقع مذهلة باستخدام HTML و CSS و JavaScript."
    },
    en: {
      title: "Web Wizardry",
  ...
  "مختبر تصميم الألعاب": {
    ar: {
      title: "مختبر تصميم الألعاب",
      description: "ابدأ في بناء ألعاب الفيديو الخاصة بك باستخدام البرمجة المرئية."
    },
    en: {
  ...
  "مبتكر التطبيقات": {
    ar: {
      title: "مبتكر التطبيقات",
      description: "صمم وانشر تطبيقات جوال حقيقية لنظامي iOS و Android."
    },
    en: {
      title: "App Inventor",
...
export function translateCourse(course: any, locale: string) {
  const translation = courseTranslations[course.title];
  
  if (translation && translation[locale as 'ar' | 'en']) {
    return {
      ...course,
      title: translation[locale as 'ar' | 'en'].title,
      description: translation[locale as 'ar' | 'en'].description,
    };
  }
...
export async function getCourses(
  ...
  try {
    ...
    // ترجمة الكورسات حسب اللغة
    data = data.map((course) => translateCourse(course, locale));
...
// ===============================
// Get Single Course with translations
// ===============================

export async function getCourseById(id: string, locale: string = 'ar'): Promise<Course | null> {
  try {
    ...
    let course: Course = await res.json();
    
    // ترجمة الكورس حسب اللغة
    course = translateCourse(course, locale);
...
export async function initializeCourses() {
  try {
    ...
    if (existingCourses.length === 0) {
      ...
      const basicProgramming = await createBasicProgrammingCourse();
...
function getWhatYouWillLearn(courseId: string): string[] {
  return ["محتوى قيد التحديث"];
}

function getRequirements(courseId: string): string[] {
  return ["لا توجد متطلبات مسبقة"];
}
...
// أضف هذه الدوال بعد باقي الدوال وقبل الـ Exports
...
> {
  try {
    ...
    // استخراج التصنيفات الفريدة من الكورسات

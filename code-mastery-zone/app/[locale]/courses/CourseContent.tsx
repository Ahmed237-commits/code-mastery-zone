...
import { getCourses, getCategories, levels, Course } from '@/app/lib/data';
import CourseCard from './components/CourseCard';
import CourseFilter from './components/CoursesFilter';
import Courses from '@/app/components/Courses';
import { getCourseFeedbacks } from '@/app/lib/feedback';
...
export default function CoursesContent() {
  ...
  const [coursesWithRatings, setCoursesWithRatings] = useState<any[]>([]);
 ...
 useEffect(() => {
    const fetchCoursesWithRatings = async () => {
      const coursesWithRatingsData = await Promise.all(
        filteredCourses.map(async (course) => {
          try {
            const ratingData = await getCourseFeedbacks(course._id);
            return {
              ...course,
      ...
      setCoursesWithRatings(coursesWithRatingsData);
    };
  ...
  // Use category.id instead of category.name for translation key
  return category ? t(`categories.${category.id}`) : filters.category;
};
            ...
            <div className="text-center max-w-4xl mx-auto">
              <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-lg rounded-full px-4 py-2 mb-6 border border-white/20">
                ...
                <span className="text-sm font-medium">
                  {filteredCourses.length} {filteredCourses.length === 1 ? t('courses') : t('courses')} {t('available')}
                </span>
              </div>
              ...
              <div className="flex flex-wrap justify-center gap-6 mt-8">
                <div className="flex items-center gap-2 bg-white/5 backdrop-blur-sm rounded-xl px-4 py-2 border border-white/10">
                  ...
                  <span className="text-sm">{t('latestCourses')}</span>
                </div>
                <div className="flex items-center gap-2 bg-white/5 backdrop-blur-sm rounded-xl px-4 py-2 border border-white/10">
                ...
                <div className="flex items-center gap-2 bg-white/5 backdrop-blur-sm rounded-xl px-4 py-2 border border-white/10">
                  ...
                  <span className="text-sm">{t('Students', { count: 5000 })}+</span>
                </div>
              </div>
            </div>
          </div>
...
{filteredCourses.length > 0 ? (
  <div className={
    ...
    {coursesWithRatings.map((course, index) => (

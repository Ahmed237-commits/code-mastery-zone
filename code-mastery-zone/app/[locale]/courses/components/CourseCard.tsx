...
export default function CourseCard({ course, variant = 'grid', t }: CourseCardProps) {
  ...
  const studentsCount = typeof course.studentsCount === 'number' 
    ? course.studentsCount 
    : Number(course.studentsCount) || 0;
  
  // تحويل rating إلى رقم بشكل آمن
  let rating = 0;
  if (typeof course.rating === 'number') {
    rating = course.rating;
  } else if (typeof course.rating === 'string') {
  ...
  // استخدام averageRating و totalRatings إذا كانت موجودة
  const averageRating = course.averageRating || rating;
  const totalRatings = course.totalRatings || 0;

  // تنسيق السعر مع الترجمة
  const formatPrice = () => {
    ...
    if (typeof course.price === 'string') {
      return course.price === '0' || course.price === 'Free' || course.price === 'مجاني' 
        ? t('details.free') 
        : `${course.price} ${t('details.price')}`;
    }
    return t('details.free');
  };

  // الحصول على اسم المستوى مترجم
  const getLevelName = () => {
    if (!course.level) return t('levels.beginner');
    return t(`levels.${course.level}`) || course.level;
  };

  // الحصول على اسم التصنيف مترجم
  const getCategoryName = () => {
    if (!course.tag) return t('categories.Programming');
    return t(`categories.${course.tag}`) || course.tag;
  };

  // الحصول على نص المدة
  const getDurationText = () => {
    if (!course.duration) return 'غير محدد';
    if (typeof course.duration === 'number') {
      return `${course.duration} ${t('hours')}`;
    }
  ...
  if (variant === 'list') {
    return (
      <div className="bg-gray-800 rounded-xl overflow-hidden hover:transform hover:scale-[1.02] transition-all duration-300 flex flex-col md:flex-row group border border-gray-700">
        <Link href={`/courses/${course._id}`} className="md:w-64 h-48 md:h-auto relative block overflow-hidden">
          ...
          {course.popular && (
            <div className="absolute bottom-2 right-2 bg-red-500 text-white px-2 py-1 rounded-lg text-xs font-bold z-10">
              {t('badges.popular')}
            </div>
          )}
        </Link>
        ...
        <div className="flex-1 p-6">
          ...
          <Link href={`/courses/${course._id}`}>
            <h3 className="text-xl font-bold mb-2 hover:text-blue-400 transition-colors line-clamp-1">
              {course.title}
            </h3>
          </Link>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              {course.originalPrice && !course.isFree && (
                <span className="text-gray-500 line-through mr-2 text-sm">{course.originalPrice} {t('details.price')}</span>
              )}
  ...
  // وضع الشبكة (grid)
  return (
    <div className="bg-gray-800 rounded-xl overflow-hidden hover:transform hover:scale-105 transition-all duration-300 group border border-gray-700 hover:border-blue-500">

...
export default function SignUpPage() {
    ...
    return (
        <div className="min-h-screen relative flex items-center justify-center bg-[#FAFAFA] dark:bg-slate-950 overflow-hidden py-10">
            {/* Background Elements */}
            <div className="absolute inset-0 z-0">
                <div className="absolute top-[-10%] right-[-10%] w-[45%] h-[45%] bg-purple-200/30 dark:bg-purple-900/10 rounded-full blur-[120px]" />
                <div className="absolute bottom-[-10%] left-[-10%] w-[45%] h-[45%] bg-indigo-200/30 dark:bg-indigo-900/10 rounded-full blur-[120px]" />
            </div>
            ...
            <div className="container mx-auto px-4 z-10">
                <div className="max-w-md mx-auto bg-white/70 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-black/20 border border-white/50 dark:border-slate-800 p-8 sm:p-10">
                    ...
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-2xl font-bold text-slate-800 dark:text-white mb-2 font-['Outfit']">{t('createAccount')}</h1>
                    ...
                    {/* Form */}
                        <form onSubmit={handleSignUpWithCredentials} className="space-y-4">
                        {error && <p className="text-red-500 text-sm">{error}</p>}
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 ml-1">{t('fullName')}</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 ml-1">{t('passwordLabel')}</label>
                            <div className="relative">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5 ml-1">{t('confirmPassword')}</label>
                            <div className="relative">
                                <i className="fas fa-check-circle absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500"></i>
                                <input
                                    type="password"
                    ...
                    <div className="mt-8 text-center">
                        <p className="text-slate-500 dark:text-slate-400 text-sm">
                            {t('hasAccount')}{' '}
                            <Link href="/signIn" className="text-indigo-600 dark:text-indigo-400 font-semibold hover:text-indigo-700 dark:hover:text-indigo-300 transition-colors">
                                {t('logIn')}
                            </Link>
                        </p>
                    </div>
